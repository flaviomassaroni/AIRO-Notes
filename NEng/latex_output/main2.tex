...
% [Formula -- da integrare con LaTeX corretto]

\begin{itemize}
  \item \textit{Capacità di membrana }(nF)
\end{itemize}
\begin{figure}[htbp]
  \centering
...
\begin{itemize}
  \item \textit{Capacità specifica di membrana }(nF/\%\%')
\end{itemize}
...
% [Formula -- da integrare con LaTeX corretto: I = dQ/dt = C\,dV/dt]
...
\begin{itemize}
  \item \textbf{Resistenza}: determina quanta corrente I è necessaria per mantenere, a regime, una diUerenza di potenziale $\Delta$';
...
\begin{itemize}
  \item -, $\rightarrow$ \textit{conduttanza specifica}
  \item -,7' $-$ 1,8 $\rightarrow$ \textit{corrente dovuta ai canali di tipo j}
  \item 1, $\rightarrow$ \textit{potenziale in cui le forze di diCusione secondo gradiente bilanciano quelle secondo gradiente elettrico per gli ioni di tipo j.}
\end{itemize}
\&\&\textbf{OSS: }anche in questo caso ci si riferisce all'unità di superficie 2! = *! .
...
\begin{itemize}
  \item -99/9 $\rightarrow$ \textit{conduttanza di perdita }(fissa e riferita al ramo di leakage)
  \item ' $\rightarrow$ \textit{potenziale di membrana}
  \item 1/ $\rightarrow$ \textit{potenziale di equilibrio }(valore medio o ``eUicace'' per più ioni)
\end{itemize}
...
Risolviamo in questo modo il problema di fitting iniziale.

% [Formula -- da integrare con LaTeX corretto]

dove: (> $\rightarrow$ \textit{tempo dell'ultimo spike}

\textit{}>C- $\rightarrow$ \textit{tempo di decadimento della conduttanza}
...
$\infty$0$\infty$$\infty$0$\infty$In questo modo introduciamo due nuove variabili ovvero $\tau$ che ha le dimensioni di un tempo ed indica quanto rapidamente n varia nel tempo, ovvero \textbf{quanto tempo impiega per raggiungere il suo valore massimo}; e n$\infty$ che è un valore costante che rappresenta il valore della probabilità di apertura dopo un lungo tempo (il valore massimo). Risolvendo l'equazione differenziale si ottiene il seguente andamento per n:  $n$($t$) = $n$  + ($n$ $-$ $n$ )$e$$-$$t$/$\tau$$n$

Ovvero per t che tende all'infinito, n tende ad n$\infty$ con \textbf{andamento esponenziale}, il tempo che n impiega per raggiungere il valore massimo è espresso proprio dalla costante di tempo $\tau$n.
...
% \subsection removed - was inline formula label
...
\textbf{$\alpha$m }rappresenta il \textbf{tasso di apertura}, moltiplicato per la probabilità che i canali siano chiusi, mentre \textbf{$\beta$m }rappresenta il \textbf{tasso di chiusura }e viene moltiplicato per la probabilità che i canali non siano chiusi, ovvero aperti.
...
Anche se la struttura neuronale reale è tridimensionale e complessa, dendriti e assone possono essere \textbf{approssimati come cilindri}, o come combinazioni di cilindri=>questo permette di ridurre il problema alla sola dimensione \textbf{longitudinale (x)}: il potenziale di membrana V dipenderà quindi dal tempo t e dalla posizione x.
...
Questa teoria nasce dall'\textbf{analogia tra un neurone e un cavo elettrico}, adattata poi alle caratteristiche specifiche della membrana cellulare.
...
Dividendo l'equazione per la superficie del cilindro (2$\pi$a$\Delta$x) e facendo tendere $\Delta$x a zero, si ottiene l'equazione dei telegrafisti, che descrive come V varia nel tempo e nello spazio.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{images/image123.jpeg}
\end{figure}
...
La \textbf{conduttanza di raccordo }rappresenta l'accoppiamento resistivo tra due compartimenti=cilindro che li collega.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{images/image128.jpeg}
\end{figure}
...
\textbf{Per osservare la dimensione temporale},ci concentriamo sul singolo compartimento.
...
=>\textbf{Scelgo un valore di x}=un certo compartimento e si blocca poi l'istante di tempo tipo \textbf{t = 9,75 ms}
...
\subsubsection{Propagazione saltatoria del potenziale d\'azione}
...
Anche in questo caso si usa un modello a compartimenti, ma con una differenza fondamentale: \begin{itemize}
  \item \textbf{Nodi di Ranvier:} sono modellati con il modello di Hodgkin--Huxley, contenente i canali voltaggio-dipendenti (limitato ai canali sodio) e sono i punti in cui il potenziale d'azione viene realmente rigenerato(conduttanza di raccordo, le conduttanze dei varai canali ecc).

  \item \textbf{Segmenti mielinizzati:} non hanno canali ionici=>non si usa né il modello di H\&H ma non hanno neanche effetti resistivi di membrana delle pompe ioniche=> non si usa modello ``integra e spara''
\end{itemize}
...
\begin{itemize}
  \item $d_m$ = spessore della membrana cellulare
  \item $c_m$ = capacità specifica di membrana
\end{itemize}
...
\begin{itemize}
  \item $2\pi a L$ = superficie del cilindro
\end{itemize}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{images/image139.jpeg}
\end{figure}
...
RisultatoPropagazione \textbf{attiva }\textit{e }continua lungo l'assone

EsempioAssone non mielinizzato modellato con 100 compartimenti

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{images/image140.jpeg}
\end{figure}

\textit{Osservazione:} il potenziale si propaga nel tempo e nello spazio
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{images/image143.jpeg}
\end{figure}
...
Struttura dell'assoneAlternanza d1 segmenti mlelinizzati \textbf{e }nodi di Ranvler

Modello fisicoGuaina - condensatori in \textbf{serie }tra cilindri concentrici
...
$\rho$($t$) = $\sum$ $\delta$($t$ $-$ $t$$i$)

$i$=1

dove n è il numero di spike, ti l'istante in cui l'i-esimo spike ha avuto luogo, i=1,2,\ldots{},n e 0 $\leq$ $t$$i$ $\leq$ $T$ dove T è la durata del trial (singola presentazione dello stimolo). $\rho$($t$) che risulta essere una somma di delta di Dirac, è detta \textbf{funzione di risposta neuronale }e descrive quando un neurone genera potenziali e non quanto grandi questi siano.
...
$\langle r \rangle = \frac{1}{T}\int_0^T r(t)\,\mathrm{d}t$ dove $\langle n \rangle$ è il numero medio di spike su tutti i trial, $T$ la durata del trial, $r(t)$ la frequenza
...
\textbf{Complessità di modelli di neurone}
...
\textbf{Reti di neuroni}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{images/image179.jpeg}
\end{figure}
...
\textbf{Connessioni cerebrali (fibre)}
...
-\textit{Metodi invasivi}, che prevedono la selezione del tessuto e il conteggio diretto delle fibre, danneggiando però il tessuto stesso;

-\textit{Metodi non invasivi}, che utilizzano immagini (spesso da risonanza magnetica) per ricostruire con precisione millimetrica il decorso delle fibre senza aprire la testa del paziente.
