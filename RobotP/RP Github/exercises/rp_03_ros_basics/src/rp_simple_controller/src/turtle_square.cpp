...
class TurtleSquareNode : public rclcpp::Node {
 ...
 private:
  ...
  void timer_callback() {
    ...
    // -Wpedantic is set in the CMake :( 
    float v = 0.0; 
    float w = 0.0;

    current_time += dt;
    
    float linear_time = side_length / trans_vel;
    float angular_time = angle_to_turn / rot_vel;

    if (state == 0){
      if (current_time>linear_time){
        state = 1;
        current_time = 0;
      }
    }
    else if (state == 1){
      if(current_time>angular_time){
        state = 0;
        current_time = 0;
      }
    }
