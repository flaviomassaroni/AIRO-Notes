...
in daily activities \cite{proietti2024combining}. Donkor \cite{donkor2018stroke} contextualize this figure in the perspective of quality of life:
...
confirms that BCI-triggered intervention outperforms conventional 
therapy (SMD\,=\,0.50), implying that conventional care alone fails 
to activate the neuroplastic mechanisms required for full motor 
recovery in the majority of patients with severe deficit.
...
(Neurolutions), closes this neural loop \cite{rustamov2025ipsihand}, but 
...
on-site clinical presence. A tablet GUI manages session setup; the 
...
(FBCSP), producing a compact spatial-spectral feature vector. 
...
the three documented sources of EEG non-stationarity in 
post-stroke patients \cite{wu2024adaptive}. Total pipeline 
latency on embedded edge hardware is $<$50\,ms, well within 
the 300\,ms Hebbian contingency window \cite{krueger2024hebbian}. 
Actuation is executed via five solenoid valves (one per digit), 
with pressure modulated by the admittance controller in the 
safety layer.
...
\begin{itemize}
    \item \textbf{Dry-electrode EEG at clinical quality:} until 2020, dry electrodes had SNR too low to reliably detect ERD on C3/C4 in real-time. Current systems (used by Premchand 2025 \cite{premchand2025personalized}) have filled this gap. Without this, self-donning at home was not possible -- a technician was needed for gel.
    \item \textbf{Real-time FBCSP inference on edge hardware:} the FBCSP+LDA decoder required around 50ms on a Raspberry Pi 4, as documented by Wu et al. \cite{wu2024adaptive}. Five years ago this required a dedicated laptop with cabled connection, which is incompatible with the home setting.
...
\end{itemize}
\smallskip
\noindent What is not yet fully mature: robust ERD decoding in patients with
extensive cortical lesions where Motor Imagery signals are
significantly reorganized remains an open engineering problem.
The regulatory framework for AI-enabled closed-loop medical devices
is evolving but not yet consolidated for home-use BCI systems.
...
serving as the \textit{predicate device} due to its identical BCI-triggered robotic paradigm and risk profile \cite{rustamov2025ipsihand}. 
...
\begin{enumerate}
    \item \textbf{Technical/Clinical: Signal Non-Stationarity.} EEG signal drift or patient-induced artifacts may degrade decoding accuracy, leading to trigger latencies exceeding the 300\,ms Hebbian window \cite{biasiucci2018brain}. 
    \textit{Mitigation:} Deployment of the \textbf{Adaptive LDA classifier} \cite{wu2024adaptive}, which performs recursive parameter updates to track neural signal changes in real-time, effectively compensating for intra-session signal drift.
    
    \item \textbf{Clinical/Safety: Spasticity-Induced Injury.} Uncontrolled spasticity during assistive movement could lead to excessive force application, potentially exceeding the 5\,N safety threshold. 
    \textit{Mitigation:} Implementation of the \textbf{admittance control loop} ($F \to v$) coupled with hardware-level \textbf{Virtual Fixtures}. The system forces the glove to yield to involuntary muscle resistance, while geometric software constraints (defined in the URDF model) prevent the actuators from exceeding the therapeutic Range of Motion (ROM).
...
\begin{itemize}
    \item \textbf{Total cortical destruction:} The system relies on the existence of residual corticospinal pathways. In cases of massive, total infarction of the primary motor cortex (M1), the Motor Imagery (MI)-based ERD signal may be too weak or absent to allow reliable BCI decoding. The device is not a substitute for functional neural substrate.
    \item \textbf{Severe cognitive impairment:} The therapy requires active patient engagement and the ability to perform mental Motor Imagery. Patients with severe post-stroke cognitive decline or dementia are currently excluded, as they lack the cognitive resources required to generate a consistent BCI intent signal.    
